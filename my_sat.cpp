/*
* Program to implement a SAT dpllInstance using the DPLL algorithm with unit
...
class Formula {
public:
	// 每个变量标记的值 a vector that stores the value assigned to each variable, where
	// -1 - unassigned
	// 0 - true
	// 1 - false
	vector<int> literals;

	// 输入的字符clause  vector to store the clauses
	...
	vector<int> literal_frequency; // 出现的频率vector to store the number of occurrences of
								   // each literal

								   // 出现的极值 vector to store the difference in number of occurrences with
...
/*
* class to represent the structure and functions of the SAT dpllInstance
*/
class SATDPLLHW1 {
private:
...
public:
	...
	void solve();      // calls the dpllInstance
};
...
void SATDPLLHW1::initialize() {
	...
	//因为是在初始化，并且是类的成员函数，所以可以直接修改
	formula.literals.clear();
	formula.literals.resize(literal_count, -1); //-1为resize之后添加的数值 标志为unassign
...
* return value: int - the status of the dpllInstance after unit resolution, a member
...
*/
int SATDPLLHW1::unit_propagate(Formula &f) {
	...
	do {
		...
		for (int i = 0; i < f.clauses.size(); i++) {
			if (f.clauses[i].size() == 1) // if the size of a clause is 1, it is a unit clause
			{
				...
				//对于unit_propagate的处理，直接将字符的数值处理为剩余的这个数值
...
*               Cat::normal - normal exit
*/
int SATDPLLHW1::apply_transform(Formula &f, int literal_to_apply) {
	...
	for (int i = 0; i < f.clauses.size(); i++) {
		...
		for (int j = 0; j < f.clauses[i].size(); j++) {
			...
			// has now become true 判断数值是否一样，如果数值一致则擦除数据
...
int SATDPLLHW1::DPLL(Formula f) {
	...
	// order to ignore them 知识为了对齐距离找到相应元素
	int i = distance(
...
/*
* function to display the result of the dpllInstance
...
*/
void SATDPLLHW1::show_result(Formula &f, int result) {
	if (result == Cat::satisfied) // if the formula is satisfiable
	{
		...
		cout << " 0" << endl;
		//cout << "final result : " << SATDPLLHW1::final_result;
	}
...
/*
* function to call the dpllInstance
*/
void SATDPLLHW1::solve() {
...
int main() {
	//printf("Test *************");
	SATDPLLHW1 dpllInstance; // create the dpllInstance
	dpllInstance.initialize();  // initialize
	dpllInstance.solve();       // solve
	return 0;
}
